...
* WebGL available and enabled.
Bloom will *not* function otherwise.
...
It is recommended to use a Windows or Mac client environment with discrete GPUs.
...
Some environments can be problematic from a visualization point of view, such as virtual or remote desktop setups.
Likewise, machines with GPU or hardware acceleration disabled can also experience visualization issues.
If you are experiencing problems while operating from one of the mentioned setups, you need to adjust your environment in order for Bloom to function properly.
Visualization issues caused by environment restrictions *cannot* be resolved by Bloom configuration changes.
